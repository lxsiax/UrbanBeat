...
export default function Productos({ productos: productosIniciales = [] }: Props) {
    ...
    const categoriasUnicas = Array.from(
        new Set(productosIniciales.map(p => p.categoria).filter(Boolean))
    );
    ...
    const productosFiltrados = productos.filter((p) => {
        ...
        const coincideNombre = nombreLimpio.includes(busquedaLimpia);
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 mb-6">
                        <div className="flex flex-col sm:flex-row gap-4 flex-grow max-w-3xl">
                            <input
                                ...
                                className="rounded-xl border-2 border-black text-xs font-black uppercase p-3 tracking-wider shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] focus:ring-0 focus:border-pink-500 bg-white flex-grow sm:flex-[2]"
                            />

                            <select
                                value={categoriaSeleccionada}
                                onChange={(e) => setCategoriaSeleccionada(e.target.value)}
                                className="rounded-xl border-2 border-black text-xs font-black uppercase p-3 tracking-wider shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] focus:ring-0 focus:border-pink-500 bg-white cursor-pointer flex-grow sm:flex-1"
                            >
